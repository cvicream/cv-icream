...
function addItem() {
  user.$patch((state) => {
    state.education.list.push(TEMPLATE_LIST_ITEM)
  })
}
...
<template>
  ...
  <div class="flex flex-col gap-6 pr-2 -mr-3 overflow-y-scroll custom-scrollbar">
    <p v-if="!education.isShow" class="paragraph text-blacks-70">
      {{ HIDDEN_INFORMATION }}
    </p>
    <div
    ...
    >
      <div class="flex justify-between items-center">
        <div>
          <h3 v-if="education.name" class="subleading text-blacks-100">
            {{ education.name[0]?.toUpperCase() + education.name.slice(1).toLowerCase() + ' ' + (index + 1) }}
          </h3>
        </div>
        <div
      ...
      >
        ...
        <div>
          ...
          <Editor
            ...
            class-name="h-[46px]"
            :enable="item.isShow"
            :placeholder="DEFAULT_TEMPLATE.education.list[index].title"
            :is-single-line="true"
          />
        </div>
        <div :class="item.isCollapsed ? 'hidden' : 'flex flex-col gap-6'">
          <div>
            ...
            <Editor
              ...
              class-name="h-[46px]"
              :enable="item.isShow"
              :placeholder="DEFAULT_TEMPLATE.education.list[index].subtitle"
              :is-single-line="true"
            />
          </div>
          <div>
            ...
            <Editor
              ...
              class-name="h-[130px]"
              :enable="item.isShow"
              :placeholder="DEFAULT_TEMPLATE.education.list[index].paragraph"
            />
          </div>
        </div>
      </div>
