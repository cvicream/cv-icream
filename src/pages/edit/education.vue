...
<template>
  <Title category="education" has-toggle-btn />
  <div class="flex-grow flex flex-col pr-2 -mr-3 overflow-y-auto custom-scrollbar last-child-pb-4">
      ...
      <div class="flex justify-between items-center">
        ...
        <div
          ...
          :class="{
            ...
            'sm:group-hover:visible': education.isShow
          }"
        >
          <Tooltip
            v-if="index !== 0 && education.list.length > 1"
            placement="bottom"
            text="Move up"
          >
          ...
          <Tooltip
            v-if="(index !== education.list.length - 1) && education.list.length > 1"
            placement="bottom"
            text="Move down"
          >
            <button @click="swap(index, index + 1)">
          ...
          >
            <div class="relative">
              ...
              >
                <button v-if="education.list.length > 1" class="flex items-center px-4 py-3 sm:hover:bg-primary-10" @click="toggleShowItem(index)">
                  <span
                ...
                <button class="flex items-center px-4 py-3 sm:hover:bg-primary-10" @click="addItem(index + 1)">
                ...
                <button v-if="education.list.length > 1" class="flex items-center px-4 py-3 sm:hover:bg-primary-10" @click="showDeleteBlockMessage(index)">
    ...
    <button
      ...
      class="w-full rounded-xl text-blacks-40 inline-flex justify-center items-center p-3 mt-6 border-transparent border-1 group bg-primary-10 sm:hover:border-primary-100"
      :disabled="!education.isShow"
      @click="addItem(null)"
    >
      <span
        class="i-custom:add w-6 h-6 text-blacks-40 flex-shrink-0"
        :class="education.isShow && 'sm:group-hover:text-blacks-70'"
      />
      <span class="subleading text-ellipsis whitespace-nowrap overflow-hidden" :class="education.isShow && 'sm:group-hover:text-blacks-100'">
        Add
      </span>
    </button>
  </div>
